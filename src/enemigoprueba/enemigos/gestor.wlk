...
object gestorEnemigos {
  ...
  method actualizar(objetivo) {
    enemigos.forEach({ e =>
      if (e.reaccionar(objetivo)) self.invocarCerca()
      e.actualizar(objetivo)
    })
  ...
  method invocarCerca() {
    if (self.puedeInvocar()) {
      const nuevo = new Sabueso(
        position = game.at(11, 9),
        campoVision = new VisionSemicircular(radio = 5),
        movimiento = new Movimiento(estadoBase = patrullarAleatorio)
      )
